...
const HomeSlider = () => {
    ...
    useEffect(()=>{
        const interval=setInterval(()=>{
            setActiveIndex((prev)=>(prev==0?1:prev==1?2:prev==2?3:prev==3?4:prev==4?5:0))
        },5000)
   ...
   <>
     <div id="carouselExampleAutoplaying" className="carousel slide " data-bs-ride="carousel">
        <div className="carousel-inner">
            ...
            <div className={`carousel-item ${activeIndex==2?"active":""}`}>
                <div className='row'>
                    ...
                    <div className="col-12 col-sm-12 col-md-6 col-lg-6"><img src="./src/assets/jaisalmer.jpeg" className="d-block w-100 img-fluid" alt="..." style={{height:"500px"}}/></div>
                </div>
            </div>
        ...
        <div className="position-absolute bottom-0 fs-5 start-50 translate-middle-x">
            {[0,1,2,3,4].map((index)=>(
                <button key={index} className='btn border-0 bg-transparent' onClick={()=>setActiveIndex(index)}>
                    <i className={`bi ${activeIndex==index?"bi-circle-fill": "bi-circle"}`} style={{fontSize:".75rem",color:"red"}}></i>
                </button>
            ))}
        </div>
    </div>
