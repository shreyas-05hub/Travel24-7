...
const toggleDislike = (type) => {
  ...
  localStorage.setItem("favoritePackageTypes", JSON.stringify(updatedFavorites));
};
  ...
  return (
    <div className='container-fluid my-4'>
      <div className="container">
        ...
        <input type="number" className='form-control my-3' value={duration} min={2} onChange={(e)=>setDuration(e.target.value)} />
        <h5 className='text-center'>Enter Budget </h5>
        <input type="range" className="form-range my-3" min={3000} max={100000} value={rangeValue} id="budgetRange" onChange={(e)=>setRangeValue(Number(e.target.value))}/>
        ...
        <div className="row">
          {destinationTypes.map((ele, i) => {
            ...
            const totalLikes = (typeFeedback.baseLikes || 0) + (typeFeedback.liked ? 1 : 0);
            const totalDislikes = (typeFeedback.baseDislikes || 0) + (typeFeedback.disliked ? 1 : 0);
                ...
                <div className="card">
                  ...
                  <div className="card-body">
                    ...
                    <p className="card-text">{ele.description}</p>
                      <button className="btn btn-primary" onClick={() => getPackage(ele)}>Get Recommendations</button>
                      <div className='my-3'>
                        <button className='btn' onClick={() => toggleFavorite(destination, ele.type)}>
                          <i className={`bi ${favorites.includes(favKey) ? 'bi-heart-fill text-danger' : 'bi-heart'}`}></i>
                        </button>
                        <button className='btn' onClick={() => toggleLike(ele.type)} style={{backgroundColor: typeFeedback.liked ? 'green' : '', color: typeFeedback.liked ? 'white' : ''}}>👍 {totalLikes}</button>
                        <button className='btn' onClick={() => toggleDislike(ele.type)} style={{backgroundColor: typeFeedback.disliked ? 'red' : '', color: typeFeedback.disliked ? 'white' : ''}}>👎 {totalDislikes}</button>
                      </div>
                  </div>
                </div>
              </div>
